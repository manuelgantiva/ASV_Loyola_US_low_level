...
// Before adding the #include "asv_interfaces/msg/state_observer.hpp" , we were using FLOAT_MULTI_64 to publish the state vector, but now we are using a custom message specially designed for the state observer.
 ...
 */
 ...
 : rclcpp::Node("ukf_multi_rate_node")
 {
   ...
   // Gravity used for IMU compensation
   ...
   Xu = [Y1u, Y2u, Y3u, Y4u, au, bu]
   Xv = [Y1v ... Y8v, av, bv, cv, dv]
   Xr = [Y1r ... Y8r, ar, br, cr, dr]
   */
   declare_parameter<std::vector<double>>("Xu", std::vector<double>(6, 1.0));
   declare_parameter<std::vector<double>>("Xv", std::vector<double>(12, 1.0));
   declare_parameter<std::vector<double>>("Xr", std::vector<double>(12, 1.0));


   /*
   Covariance matrices are passed as flattened row-major arrays.
 ...
 */
 ...
 At each IMU step:
   1) Copy the latest low-rate packet
   2) Always do IMU-only update with [ax ay r]
   3) If a NEW low-rate packet arrived, also do full update with
      [x y psi ax ay r]
   4) Save posterior for publishing
   5) Predict one step using the latest held delta values


 So:
 - x,y,psi are used only once when they are fresh
 - delta_* remain constant automatically until new packet arrives
 */
 void runFilterCycle(const ImuData & imu_local)
 {
   // ------------------------------------------------------------------------
   // 1) COPY LOW-RATE DATA SAFELY
   // ------------------------------------------------------------------------
   {
     std::lock_guard<std::mutex> lock(data_mutex_);


     // Snapshot of last low-rate packet for this cycle
     observer_cycle_ = observer_latest_;


     // Was there a new low-rate packet since last IMU cycle?
     low_rate_update_this_cycle_ = have_fresh_low_rate_;


     // Consume the flag so x,y,psi are not reused as fresh again
     have_fresh_low_rate_ = false;
   }

...
public:
  ...
  */
  ...
  : rclcpp::Node("multi_ukf_unarmed_node")
  {
    // ------------------------------------------------------------------------
    // 1) DECLARE PARAMETERS
    // ------------------------------------------------------------------------
    ...
    // Ignore all sensor data for this many seconds after the first
    // observer/data_sensors packet is received. The UKF is then initialized
    // from the latest observer packet and starts running from IMU callbacks.
    declare_parameter<double>("startup_delay_sec", 10.0);
    ...
    */
    ...
    declare_parameter<double>("alpha", 0.3);
    declare_parameter<double>("beta",  2.0);
    declare_parameter<double>("kappa", 0.0);
    ...
    startup_delay_sec_ = get_parameter("startup_delay_sec").as_double();

    if (!std::isfinite(startup_delay_sec_) || startup_delay_sec_ < 0.0) {
      throw std::runtime_error(
        "Parameter 'startup_delay_sec' must be finite and nonnegative.");
    }
    ...
    publisher_state_estimate_ = create_publisher<asv_interfaces::msg::StateObserver>(
      "/" + my_id_ + "/observer/state_observer_ukf_unarmed",
      rclcpp::SensorDataQoS());
    ...
    publisher_state_estimate_lowrate_ = create_publisher<asv_interfaces::msg::StateObserver>(
      "/" + my_id_ + "/observer/state_observer_ukf_lowrate_unarmed",
      rclcpp::SensorDataQoS());
    ...
    */
    ...
    RCLCPP_INFO(
      get_logger(),
      "UKF is independent of MAVROS armed/disarmed state");
    RCLCPP_INFO(
      get_logger(),
      "Waiting %.3f s after the first /%s/observer/data_sensors packet "
      "before UKF initialization",
      startup_delay_sec_, my_id_.c_str());
    RCLCPP_INFO(get_logger(), "observer/data_sensors is assumed low-rate");
...
private:
  ...
  */
  ...
  /*
  ============================================================================
  CALLBACK: observer/data_sensors
  ============================================================================
  Expected order:
    [0] x
    [1] y
  ...
  */
  void callbackObserverData(const std_msgs::msg::Float32MultiArray::SharedPtr msg)
  {
    ...
    // Start the 10-second (parameterized) delay from the first received
    // data_sensors packet. During the delay, packets are stored but are not
    // used by the UKF.
    if (!observer_warmup_started_) {
      observer_warmup_started_ = true;
      observer_warmup_start_ = steady_now;

      RCLCPP_WARN(
        get_logger(),
        "First data_sensors packet received. Starting %.3f s UKF delay.",
        startup_delay_sec_);
    }

    if (!startup_delay_complete_) {
      const double elapsed_sec = std::chrono::duration<double>(
        steady_now - observer_warmup_start_).count();

      if (elapsed_sec >= startup_delay_sec_) {
        startup_delay_complete_ = true;

        RCLCPP_WARN(
          get_logger(),
          "UKF startup delay complete after %.3f s. "
          "The next IMU callback will initialize the filter from the latest "
          "data_sensors packet.",
          elapsed_sec);
      } else {
        RCLCPP_INFO_THROTTLE(
          get_logger(), *get_clock(), 1000,
          "Collecting data_sensors during startup: %.2f / %.2f s",
          elapsed_sec, startup_delay_sec_);
      }
    }


    // This flag is consumed once in the next IMU cycle after startup.
  ...
  */
  void callbackImuData(const sensor_msgs::msg::Imu::SharedPtr msg)
  {
    ...
    // The UKF does not use IMU or observer measurements during the startup
    // delay. The delay begins when the first data_sensors packet arrives.
    bool startup_ready = false;
    double remaining_sec = startup_delay_sec_;

    {
      std::lock_guard<std::mutex> lock(data_mutex_);
      have_received_imu_ = true;
      startup_ready = startup_delay_complete_;

      if (observer_warmup_started_ && !startup_delay_complete_) {
        const auto elapsed = std::chrono::duration<double>(
          std::chrono::steady_clock::now() - observer_warmup_start_).count();
        remaining_sec = std::max(0.0, startup_delay_sec_ - elapsed);
      }
    }
    ...
    if (!startup_ready) {
      RCLCPP_INFO_THROTTLE(
        get_logger(), *get_clock(), 1000,
        "UKF startup delay active: waiting %.2f more seconds after first "
        "data_sensors packet",
        remaining_sec);
      return;
    }
  ...
  */
  void initializeIfNeeded()
  {
    ...
    if (!startup_delay_complete_ ||
        !have_received_imu_ ||
        !observer_latest_.valid ||
        !have_fresh_low_rate_)
    {
      RCLCPP_INFO_THROTTLE(
        get_logger(), *get_clock(), 2000,
        "UKF waiting for inputs: delay_complete=%s, imu_received=%s, "
        "observer_valid=%s, fresh_low_rate=%s",
        startup_delay_complete_ ? "true" : "false",
        have_received_imu_ ? "true" : "false",
...
private:
  ...
  */
  ...
  double startup_delay_sec_ = 10.0;
  ...
  */
  ...
  // Startup is controlled only by receipt of observer/data_sensors and IMU.
  // No armed/disarmed state is used.
  bool observer_warmup_started_ = false;
  bool startup_delay_complete_ = false;
  bool have_received_imu_ = false;
  std::chrono::steady_clock::time_point observer_warmup_start_;

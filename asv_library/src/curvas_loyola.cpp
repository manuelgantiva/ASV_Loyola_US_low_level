...
Target curva_1_i(const float w) {
    ...
    if (w < 10.077822185373194) {
        ...
        arco(w - 10.077822185373194 + 2.7334435498347043, &result, -18.793822301370934, -1.6642859280797144,  2.0, false);
    }
    else if (w >= 11.648618512 && w < 21.898618512) {
...
Target curva_1_c(const float w) {
    ...
    if (w < 5.038911092686587) {
        ...
        // Trayectoria 2: Cuarto de circunferencia 
        arco(w - 5.038911092686587 + 2.7334435498347034, &result, -18.79764460274187, -1.6685718561594307, 4.0, false);
    }
    else if (w >= 6.609707417 && w < 11.73470742) {
        // Trayectoria 3: Línea recta 
...
Target curva_1_d(const float w) {
    ...
    if (w < 3.3592740617910617) {
        ...
        // Trayectoria 2: Cuarto de circunferencia 
        arco(w - 3.3592740617910617 + 2.733443549834704, &result, -18.801466904112804, -1.6728577842391446, 6.0, false);
    }
    else if (w >= 4.930070389 && w < 8.347070389) {
        // Trayectoria 3: Línea recta 

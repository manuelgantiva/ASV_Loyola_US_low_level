...
class DataConverter:
    """Utilities for converting between ROS messages, NumPy arrays, and
    PyTorch tensors."""
    ...
    def scale_action_to_physical(action_normalized, action_type):
        ...
        if action_type == "vyaw":
            return action_normalized * 0.5  # Scale to [-0.5, 0.5] rad/s (legacy compatibility)
        elif action_type == "acceleration":
            return action_normalized * 0.1  # Scale to [-0.1, 0.1] m/s² (legacy compatibility)

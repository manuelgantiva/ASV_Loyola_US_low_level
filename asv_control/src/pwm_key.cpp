...
public:
  TeleopTurtle()
  {
    nh_ = rclcpp::Node::make_shared("teleop_key");
  ...
  int keyLoop()
  {
    ...
    while (running)
    {
      ...
      if (running && (linear != 0.0 || angular != 0.0))
      {
        ...
        */
        asv_interfaces::msg::PwmValues pwmvalues;
        pwmvalues.t_left = left;
        pwmvalues.t_righ = right;
        pwm_pub_->publish(pwmvalues);
      }
    }
...
private:
  ...
  rclcpp::Publisher<asv_interfaces::msg::PwmValues>::SharedPtr pwm_pub_;

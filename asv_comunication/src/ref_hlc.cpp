...
class RefHlcNode : public rclcpp::Node
{
public:
    RefHlcNode() : Node("ref_hlc") 
    {
        std::string my_id; 
        ...
        params_callback_handle_ = this->add_on_set_parameters_callback(std::bind(&RefHlcNode::param_callback, this, _1));

        subscriber_rc_in = this-> create_subscription<mavros_msgs::msg::RCIn>("/" + my_id + "/mavros/rc/in",10,
                std::bind(&RefHlcNode::callbackRcIn, this, std::placeholders::_1));
        subscriber_mavros_state = this-> create_subscription<mavros_msgs::msg::State>("/" + my_id + "/mavros/state",1,
                std::bind(&RefHlcNode::callbackMavrosState, this, std::placeholders::_1));
        publisher_ref = this-> create_publisher<std_msgs::msg::Float64>("/" + my_id + "/control/reference_hlc",1);
    	RCLCPP_INFO(this->get_logger(), "Reference High level control Node in %s has been started.", my_id.c_str());
    }
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<RefHlcNode>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}